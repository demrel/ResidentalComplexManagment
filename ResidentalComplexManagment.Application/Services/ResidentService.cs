...
namespace ResidentalComplexManagment.Application.Services
{
    public class ResidentService : IResident
    {
        ...
        public async Task<List<ResidentDebtItemDTOListItem>> GetDebtItems(string residentId)
        {
            ...
            var debtitems = await _debItemRepository.ListAsync();

            List<ResidentDebtItemDTOListItem> list = new();
            foreach (var item in debtitems)
            {
                list.Add(new ResidentDebtItemDTOListItem()
                {
                    Name = item.Name,
                    IsActive = item.IsComplusory,
                    IsCheckid = paymentItemIdList.ContainsKey(item.Id),
